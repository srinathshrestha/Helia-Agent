      ...
      <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-2">
        <div className="flex items-center gap-0.5">
          <Button
            variant="ghost"
            size="sm"
            className={`h-8 px-2 md:px-3 rounded-r-none border-r border-gray-700/30 transition-colors duration-200
              ${liked ? 'text-green-500 hover:text-green-400 hover:bg-green-500/10' : 
                     'text-gray-400 hover:text-gray-300 hover:bg-gray-700/30'}`}
            onClick={handleLike}
            title="Like this response"
          >
            <ThumbsUp className={`h-4 w-4 ${!liked ? 'md:mr-1.5' : ''} ${liked ? 'fill-green-500' : ''}`} />
            <span className="hidden md:inline">{liked ? 'Liked' : 'Like'}</span>
          </Button>
          <Button
            variant="ghost"
            size="sm"
            className={`h-8 px-2 md:px-3 rounded-l-none transition-colors duration-200
              ${disliked ? 'text-red-500 hover:text-red-400 hover:bg-red-500/10' : 
                        'text-gray-400 hover:text-gray-300 hover:bg-gray-700/30'}`}
            onClick={handleDislike}
            title="Dislike this response"
          >
            <ThumbsDown className={`h-4 w-4 ${!disliked ? 'md:mr-1.5' : ''} ${disliked ? 'fill-red-500' : ''}`} />
            <span className="hidden md:inline">{disliked ? 'Disliked' : 'Dislike'}</span>
          </Button>
        </div>
        ...
        <div className="flex items-center gap-0.5">
          <Button
            ...
            size="sm"
            className={`h-8 px-2 md:px-3 rounded-r-none border-r border-gray-700/30 transition-colors duration-200
              text-gray-400 hover:text-gray-300 hover:bg-gray-700/30 active:scale-95`}
            onClick={handleCopy}
          ...
          <Button
            ...
            title="Read aloud"
          >
            <Volume2 className={`h-4 w-4 ${!isPlaying ? 'md:mr-1.5' : ''} ${isPlaying ? 'fill-primary/20' : ''}`} />
            <span className="hidden md:inline">{isPlaying ? 'Stop' : 'Read'}</span>
          </Button>
        </div>
      </div>
    </div>
